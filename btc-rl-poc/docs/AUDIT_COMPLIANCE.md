...
**Operational:** ~30h system-wide capture outage 09-19→09-20 (all live arms; resumed 09-20 14:xx). Schema drift across arms: `late_settle_ts` only in pt; `p_arm` (pt/tv/cg33) vs `p_up` (ob/fm); ob/fm add model/quantile fields. Fixed: `docs/TRADE_LEDGER_SCHEMA.md` + `economics.arm_p()` accessor.

**GUARDRAILS WERE DARK — now running (biggest systemic gap, root cause found).** `pytest tests/` returned **"no tests ran"**: `tests/test_a3_scenarios.py:113` (and others) call `sys.exit()` at MODULE level, aborting pytest COLLECTION for the whole suite (INTERNALERROR). So every guardrail test was never executed — they gated nothing. Root `conftest.py` now `collect_ignore`s the 8 script-style `test_*.py` (no `def test_`, some module-level exit; still runnable as `python tests/<f>.py`). Suite now collects → **133 passed, 3 failed** (was 0 runnable). Failures fixed vs. deferred:
  - FIXED (test-hygiene, not lowering the bar): `test_m5_system` × 2 — symlinked a SHARED `/tmp/results` that persisted across sessions/jobs (a dangling symlink from Aug 30 caused FileExists/FileNotFound); rewrote hermetic (nested `results/`, pinned `fw.ROOT`) + corrected a filename drift (`a3_window_evaluation`→`a3v21_window_evaluation`). `test_core::test_reward_spec` — the `$50→-0.5` shaped-reward assert was stale vs the documented "three-bands-out→-1" ramp (HIT_BAND=5 ⇒ $50 is u=10 ⇒ clamped -1); STRENGTHENED to check both the ramp midpoint ($12.5→-0.5) and the clamp.
  - DEFERRED to owner (needs a judgment call, NOT a blind fix): `test_architecture_checkpoint::test_runtime_contract_truth_three_states` — asserts exact-BRTI runtime settlement, which is OFF pending the DT-01 daemon activation (owner restart); correctly red. `test_hf::test_t11_ctx_dim_matches_t2` — t11 ctx dim is 18, test expects 10 (==t2). t11 carries `hf:True`; is HF meant to affect REWARD only (then t11 regressed — extra features contaminate the controlled t2 comparison) or ADD features (then the test is stale)? Owner call — flagged, not guessed.

Still LIVE-only / restart-gated (unchanged): `online.py` dead retired-arm code + fail-closed-covers-all-arms + the DT-01 runtime flag. Not touched from a background job against the running daemon.
